...
colData(pe)$sample_type <- metadata$Response.state
...
contrast_names <- c("sample_typepoor")
...
for (contrast in contrast_names) {
  ...
  volcano_plot <- ggplot(volcano_data, aes(x = logFC, y = negLogAdjPval, color = significant)) +
    ...
    print(volcano_plot)
    ggsave(filename = paste0("/home/compomics/git/Projects/MLMarker_paper/Figures_paper/volcano_plot_", contrast, ".png"), plot = volcano_plot, width = 8, height = 6, dpi = 300)
    csv_output <- results[, c("Protein.Groups", logFC_col, adj_pval_col), drop = FALSE]
    write.csv(
      csv_output,
      file = paste0("/home/compomics/git/Projects/MLMarker_paper/Figures_paper/volcano_data_", contrast, ".csv"),
      row.names = FALSE
    )
  }
